    ...
    if (error) {
      ...
      sharp(body)
        ...
        .toFile(`public/img/${request.uri.split("/").pop()}.avif`, () => {
          COMPLETE_CNT += 1;
          if (COMPLETE_CNT % 10 === 0) {
            logger(
              `IMAGES [${COMPLETE_CNT}/${GLOBAL_INDEX_DB.size}] completed.`,
            );
          }
          if (COMPLETE_CNT === GLOBAL_INDEX_DB.size) {
            logger(
              `IMAGES [${COMPLETE_CNT}/${GLOBAL_INDEX_DB.size}] completed.`,
            );
          }
...
const BangumiCrawler = new Crawler({
  ...
  callback: async (error, { request, $ }, done) => {
    if (error) {
      ...
      const statusTextLong = getStatusTextLong(end, episodes);
      const statusText = getStatusText(end, episodes);
      const updated = now();
