...
const ImageCrawler = new HttpCrawler(
  {
    ...
    requestHandler: async ({ request, body, response }) => {
      try {
        const img = `${request.url.split("/").pop()}.avif}`;
...
async function copyImgFromCache (imgs) {
  await Promise.all(imgs.map(async(img)=>{
    if(existsSync(join(CACHE_DIR_PATH, img))){
      await copyFile(join(CACHE_DIR_PATH, img), join(IMG_DIR_PATH, img));
    } else {
      logger(`[CACHE] cannot found ${join(CACHE_DIR_PATH, img)} in cache, skip copy`)
    }
  }));
}
...
export default async function crawler() {
  try {
    ...
    const currentImgs = [...GLOBAL_INDEX_DB.values()].map(({ img }) => img);
    const imgsFromSource = await restoreImgCache(currentImgs);

    logger(`Fetch Bangumi Images, ${imgsFromSource.length} images needs to fetch...`);
