...
function pruneImgUrls (cachedImgUrls, currentImgUrls) {
  ...
  const imgUrlsFromCache = [...currentImgUrlsSet].filter(img => cachedImgUrlsSet.has(img));
  const imgUrlsFromSource = [...currentImgUrlsSet].filter(img => !cachedImgUrlsSet.has(img));
...
async function restoreImgCache(currentImgUrls) {
  ...
  const {imgUrlsFromCache, imgUrlsFromSource, imgUrlsAll } = pruneImgUrls(cachedImgUrls, currentImgUrls);
...
export default async function crawler() {
  try {
    ...
    const allImgUrlsSize = GLOBAL_IMG_URLS_SET.size;
    const imgsUrlsToFetch = await restoreImgCache([...GLOBAL_IMG_URLS_SET]);

    logger(`Fetch Bangumi Images, ${imgsUrlsToFetch.length} images needs to fetch...`);
    await ImageCrawler.run(imgsUrlsToFetch);
    logger(`Complete fetch ${imgsUrlsToFetch - (allImgUrlsSize - GLOBAL_IMG_URLS_SET.size)} images`);
  } catch (err) {
