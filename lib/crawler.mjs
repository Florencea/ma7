...
async function copyImgFromCache (imgs) {
  await Promise.all(imgs.forEach(async (img)=>{
...
async function restoreImgCache(currentImgs) {
  ...
  logger(`Complete restore ${fromCache} images`);
  writeImgCacheList(all);
  logger(`Complete write ${all} images to cache list`);
  return fromSource;
}
...
export default async function crawler() {
  try {
    ...
    logger(`Fetch Bangumi Images, ${imgsFromSource} images needs to fetch...`);
