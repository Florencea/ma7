 ...
 * @type {Set<string>}
 */
const GLOBAL_IMG_URLS_SET = new Set();
 ...
 * @param {string} imgUrl
 */
function getImgNameFromUrl(imgUrl) {
  return `${imgUrl.split("/").pop()}.avif`;
}
...
function getBangumiRoadshow($) {
  ...
  return roadshow === "Invalid Date" ? "未知" : roadshow;
}
...
const ImageCrawler = new HttpCrawler(
  {
    ...
    requestHandler: async ({ request, body, response }) => {
      try {
        const img = getImgNameFromUrl(request.url);
        ...
        GLOBAL_IMG_URLS_SET.delete(request.url);
        logger(
...
const BangumiCrawler = new CheerioCrawler(
  {
    ...
    requestHandler: async ({ request, $ }) => {
      ...
      const img = getImgNameFromUrl(imgUrl);
      ...
      GLOBAL_IMG_URLS_SET.add(imgUrl);
    },
  },
  new Configuration(CRAWLER_OPTIONS),
);
 ...
 **/
function pruneImgList (cached, current) {
  const cachedSet = new Set(cached);
  const currentSet = new Set(current);
  const fromCache = [...currentSet].filter(img => cachedSet.has(img));
  const fromSource = [...currentSet].filter(img => !cachedSet.has(img));
  const all = [...new Set([...fromCache, ...fromSource])];
  return {fromCache, fromSource, all};
}

/**
 * @param {string[]} imgs 
 */
async function copyImgFromCache (imgs) {
  await Promise.all(imgs.map(async(img)=>{
    const imgpath = `${img.split("/").pop()}.avif`;
    const source = join(CACHE_DIR_PATH, imgpath);
    const destination = join(IMG_DIR_PATH, imgpath);
 ...
 * @param {string[]} imgs 
 */
function writeImgCacheList (imgs) {
  const imgsTxt = imgs.join("\n");
  writeFileSync(CACHE_FILE_PATH, imgsTxt);
}

/**
 * 
 * @param {string[]} currentImgs 
 */
async function restoreImgCache(currentImgs) {
  const cachedImgs = loadCachedImgList();
  logger(`Load cache list: ${cachedImgs.length} images`);
  const {fromCache, fromSource, all } = pruneImgList(cachedImgs, currentImgs);
  logger(`Restore from cache...`);
  await copyImgFromCache(fromCache);
  logger(`Complete restore ${fromCache.length} images`);
  writeImgCacheList(all);
  logger(`Complete write ${all.length} images to cache list`);
  return fromSource;
}
...
export default async function crawler() {
  try {
    ...
    const allFetchSize = GLOBAL_IMG_URLS_SET.size;
    const imgsFromSource = await restoreImgCache([...GLOBAL_IMG_URLS_SET]);

    logger(`Fetch Bangumi Images, ${willFetchSize} images needs to fetch...`);
    await ImageCrawler.run(imgsFromSource);
    logger(`Complete fetch ${imgsFromSource - (allFetchSize - GLOBAL_IMG_URLS_SET.size)} images`);
  } catch (err) {
